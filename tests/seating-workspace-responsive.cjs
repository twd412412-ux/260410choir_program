...
(async () => {
  ...
  try {
    ...
    for (const viewport of profiles) {
      ...
      await usable('.seating-structure-toolbar [aria-label="한줄칸 추가"]');
      await usable('.seating-structure-toolbar [aria-label="한줄칸 제거"]');
      await usable('.seating-structure-toolbar [aria-label="공개본 비교"]');
