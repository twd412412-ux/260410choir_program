...
(async () => {
  ...
  try {
    ...
    const checks = await page.evaluate(parts => {
      ...
      const upcoming = allSongs[0];
      const oldBlessing = { id: 'song-aug26', songName: '축복하노라', year: 2026, month: 8, day: 26, category: '교구집회' };
      const stale = { ...parts[0], id: 'stale-blessing', title: '축복하노라', scoreKind: 'singer', linkedSongId: upcoming.id, linkedSongIds: [upcoming.id], linkedSongName: '축복하노라', versionNumber: 2 };
      const singer = { ...parts[0], id: 'el-singer', title: 'El Shaddai', scoreKind: 'singer' };
      allSongs = [upcoming, oldBlessing]; allScores = [...parts, stale, singer];
      check(!scoreHasCurrentSongLink(stale, upcoming), 'renamed song kept stale ID link');
      check(findScoresForSong(upcoming, true).length === 21, 'stale singer shown under El Shaddai');
      check(findScoresForSong(oldBlessing, true).some(score => score.id === stale.id), 'correct historical score hidden');
      check(findSongForScore(stale).id === oldBlessing.id, 'reverse detail points to wrong future song');
      check(findExplicitSongForScore(stale) === null, 'explicit resolver trusts stale ID');
      check(scoreLinkedSongs(stale).length === 0, 'summary still describes invalid connection');
      check(findScoreUploadReplacement('엘샤다이', 'singer', '', [upcoming.id], {}, stale.id, '') === null, 'stale link could replace another song file');
      check(scoreHasCurrentSongLink({ ...stale, linkedSongName: upcoming.songName }, upcoming), 'intentional cross-title link blocked');
      check(scoreHasCurrentSongLink({ ...stale, linkedSongName: '' }, upcoming), 'legacy link without title was changed');
      check(scoreHasCurrentSongLink({ ...stale, linkedSongId: 'primary', linkedSongIds: ['primary', upcoming.id] }, upcoming), 'primary title wrongly applied to secondary link');
      check(scoreHasCurrentSongLink({ ...singer, linkedSongId: upcoming.id, linkedSongIds: [upcoming.id], linkedSongName: '엘샤다이' }, upcoming), 'bilingual correction broke valid link');
      document.getElementById('scoreTitle').value = stale.title;
      setScoreLinkedSongs(stale.linkedSongIds, stale.linkedSongName, false);
      check(document.getElementById('scoreLinkSummary').textContent === '연결 확인 필요', 'stale editor warning missing');
      openScorePickerForSong(upcoming.id, false);
      check(!document.getElementById('scorePickerBody').textContent.includes('축복하노라'), 'picker still displays stale singer');
      check(document.getElementById('scorePickerBody').textContent.includes('El Shaddai'), 'correct singer missing from picker');
      closeModal('modalScorePicker');
      allScores = parts; allSongs = [upcoming];
