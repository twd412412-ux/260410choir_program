  ...
  try {
    for (const engine of [chromium, webkit]) {
      ...
      try {
        ...
        const times = page.locator('.concert-duration-row input');
        for (let i = 0; i < 3; i++) { await times.nth(i).fill('4:30'); await times.nth(i).press('Tab'); }
        ...
        await page.locator('.concert-duration-row input').nth(1).fill('1:00'); await page.locator('.concert-duration-row input').nth(1).press('Tab');
