...
(async()=>{
  ...
  sh.getCell('C6').value='윤하은';assert.throws(()=>core.parse(w,members),/칸 밖/);sh.getCell('C6').value=null;
  sh.getCell('B6').value=4;assert.throws(()=>core.parse(w,members),/칸수/);sh.getCell('B6').value=3;
  ...
  for(const counts of [[35,36,37,36,36],[10,10,10],[9,10,8],[1],[120]]){
    ...
    assert.deepEqual(core.parse(await disk(core.build(ExcelJS,p,members)),members).snapshot.rows,p.rows);
  }
